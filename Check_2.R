...
rP_new <- newPoisson(P, nSample=5e5, ThetaSamples = 320, RhoSamples = 135)
rP <- matrix(scan("PETcodes_Maitra/Hoff.fwd"), 128)

# Original noisy sinograms
# par(mfrow = c(1, 2))
# image(rP)
# image(rP_new)
...
Kty <- matrix(scan("PETcodes_Maitra/Kty.dat"), 128)
eigens <- matrix(scan("PETcodes_Maitra/eigens_Maitra.dat"), 128)
