...
July  06, J.Schulz Included KernelFileSave
***********************************************************************/
SparseMatrix *GenerateAMatrix()
{
  ...
  if (!(NewMatrix=ReadAMatrix())) {
    ...
    for(t=0; t<ThetaSamples; t++ ) {
      Print(_DNormal,"Calculating: %4d of %d \r",t,ThetaSamples);
...
March 96, JJJ and PT\\
April 2, 96 PT Uses itini.h defs
March 07, J.Schulz, Modification in case P1 thetaar[r] to thetaar[t]
***********************************************************************/
Vector *GenerateAMatrixRow(int rowindex)
{
  ...
  switch (itINI.RadonKernel)
  {
	...
	for (n=0; n<YSamples; n++){
	  ...
	  if ((intm>=0) && (intm<(XSamples-1))) {
	    ...
	    tempV[intm+1+n*XSamples]=SquareRadon(rho,thetaar[t],xar[intm+1],yar[n]);	      
	  }
	  realm-=cst3;
	}
      }
...
Vector *GenerateAMatrixColumn(int colindex)
{
  int m,n,r,t,M;
  //int intn,intm;
  float onedivsq2,psi,pidivDeltaX,costheta,sintheta,of;
  //float cst1,cst2,realm,realn,deltan,deltam;
  float *tempV;
  Vector *NewVector;
...
/* The RNN and RL algorithm are inactive, because there is a mistake in the
   algorthim, which I did not found until now. If you activate this part you have to activate the above integer and float variables and to deactivate the below temporary settings for RNN and RL. If you find the mistake, please send me an email <jschulz78@web.de> and I will it work into the PET-package. */
...
/*    case _RNN: // ray driven nearest neighbor
    ...
    case _RL: // ray driven linear interpolation
      ...
      break;
*/    
    case _RNN: // temporay setting !!!!!!!!!!!!!!!!!!!!
    case _RL:  // temporay setting !!!!!!!!!!!!!!!!!!!!
